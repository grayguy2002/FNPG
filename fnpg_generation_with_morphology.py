...
# ============================================================================
# R2-3.5: 形态学保真度验证 (Morphological Fidelity Validation)
# ============================================================================
print("\n" + "="*70)
print("R2-3.5: 形态学保真度验证")
print("="*70)
...
try:
    ...
    try:
        ...
        print(f"  ✗ 缺少依赖: {ie}")
        print("  请运行: !pip install fastdtw scikit-image")
        raise
    ...
    # 提取标准化的目标数据
    target_gcrr = tensor_list_target[well_idx][:, 2:, :].clone()  # RSHA, RMED
    target_grsb = tensor_list_target_2[well_idx][:, 1:3, :].clone()  # RDEP, SP
    standardized_target = torch.cat((target_gcrr, target_grsb), dim=1)

    # 对目标数据也进行逆变换
    target_feature_names = ['RSHA', 'RMED', 'RDEP', 'SP']

    destandardized_target_list = []
    for i, feat_name in enumerate(target_feature_names):
        if i < standardized_target.shape[1]:
            dest_feat = inverse_transformer.inverse_transform(
                standardized_target[:, i, :],
                feat_name
            )
            destandardized_target_list.append(dest_feat)

    destandardized_target = torch.stack(destandardized_target_list, dim=1)

    print(f"  预测数据形状: {destandardized_pred.shape}")
    print(f"  目标数据形状: {destandardized_target.shape}")

    # 转换为 numpy 并处理维度差异
    pred_np = destandardized_pred.cpu().numpy() if torch.is_tensor(destandardized_pred) else destandardized_pred
    target_np = destandardized_target.cpu().numpy() if torch.is_tensor(destandardized_target) else destandardized_target
    ...
    if pred_np.ndim == 2:
        # 预测是 (C, L) 格式
        print(f"  预测数据是 2D (C, L) 格式,将与目标数据的所有样本对比")
        n_features = pred_np.shape[0]

        # 对目标数据,拼接所有样本以匹配预测的长度
        # 目标: (N, C, L) -> (C, N*L)
        target_np_reshaped = target_np.transpose(1, 0, 2).reshape(target_np.shape[1], -1)

        print(f"  重塑后 - 预测: {pred_np.shape}, 目标: {target_np_reshaped.shape}")

        # 对齐长度 (取最短)
        min_len = min(pred_np.shape[1], target_np_reshaped.shape[1])
        pred_np_aligned = pred_np[:, :min_len]
        target_np_aligned = target_np_reshaped[:, :min_len]

    elif pred_np.ndim == 3:
        # 预测是 (N, C, L) 格式
        print(f"  预测数据是 3D (N, C, L) 格式")

        # 对齐样本数
        min_samples = min(pred_np.shape[0], target_np.shape[0])
        pred_np_aligned = pred_np[:min_samples]
        target_np_aligned = target_np[:min_samples]

        # 拼接所有样本: (N, C, L) -> (C, N*L)
        pred_np_aligned = pred_np_aligned.transpose(1, 0, 2).reshape(pred_np_aligned.shape[1], -1)
        target_np_aligned = target_np_aligned.transpose(1, 0, 2).reshape(target_np_aligned.shape[1], -1)

        n_features = pred_np_aligned.shape[0]
    else:
        raise ValueError(f"不支持的预测数据维度: {pred_np.ndim}")

    print(f"  最终对齐 - 预测: {pred_np_aligned.shape}, 目标: {target_np_aligned.shape}")
    ...
    def compute_dtw(pred, target):
        ...
        # 使用简化方法: 归一化的欧氏距离 (作为DTW的近似)
        # 这避免了fastdtw的维度问题
        diff = pred - target
        distance = np.sqrt(np.mean(diff ** 2))  # RMSE

        # 归一化到[0,1]范围
        max_range = max(np.ptp(pred), np.ptp(target))
        if max_range > 0:
    ...
    def compute_ssim(pred, target):
        ...
        # 归一化到[0,1]
        pred_norm = normalize_to_01(pred)
        target_norm = normalize_to_01(target)

        # 计算SSIM
        return ssim_func(pred_norm, target_norm, data_range=1.0, win_size=11)

    # 计算每个特征的指标
    results = []

    print("\n计算形态学指标:")
    for i, feat_name in enumerate(target_feature_names[:n_features]):
        # 现在数据是 (C, L) 格式,直接索引即可
        pred_feat = pred_np_aligned[i, :]
        target_feat = target_np_aligned[i, :]

        dtw_dist = compute_dtw(pred_feat, target_feat)
        ssim_val = compute_ssim(pred_feat, target_feat)
        ...
        print(f"  {feat_name}: DTW={dtw_dist:.4f}, SSIM={ssim_val:.4f} ({fidelity})")

    results_df = pd.DataFrame(results)
    mean_dtw = results_df['DTW'].mean()
    ...
    # 生成 LaTeX 表格代码
    print("\n" + "="*70)
    print("LaTeX 表格代码 (复制到论文 Table S4_morphology)")
    print("="*70)
    print("\n注意: DTW使用了归一化RMSE作为近似指标")

    latex_code = """
\\begin{table}[h!]
\\centering
\\caption{Morphological fidelity validation of inverse-transformed predictions on test well 15/9-15. Normalized RMSE (Root Mean Square Error) measures shape similarity (lower is better, normalized to 0-1 scale), while Structural Similarity Index (SSIM) quantifies local pattern preservation (higher is better, range 0-1). High SSIM values and low RMSE indicate that despite the theoretical ill-posedness of the single-step inverse transformation, FNPG maintains strong morphological consistency with ground truth curves.}
\\label{tab:S4_morphology}
\\small
\\begin{tabular}{l c c c}
\\toprule
\\textbf{Feature} & \\textbf{Norm. RMSE} & \\textbf{SSIM} & \\textbf{Morphological Fidelity} \\\\
\\midrule
"""

    for _, row in results_df.iterrows():
        latex_code += f"{row['Feature']} & {row['DTW']:.3f} & {row['SSIM']:.3f} & {row['Fidelity']} \\\\\n"

    latex_code += f"""\\midrule
\\textbf{{Mean}} & \\textbf{{{mean_dtw:.3f}}} & \\textbf{{{mean_ssim:.3f}}} & -- \\\\
\\bottomrule
\\end{{tabular}}
\\end{{table}}
"""
